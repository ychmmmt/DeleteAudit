...
public sealed class LiveMonitoringViewModel : ViewModelBase, IDisposable
{
    ...
    public const string PersistenceDisclosure =
        "Phase 2B 实时接入：你手动开始后，本次接收到的受支持事件原始 XML、解析与分类结果"
        + "及相关实时证据会写入本机查看器数据库。批次达到 64 条时立即进入持久化；未满批次从第一条"
        + "进入空批次起通常约 5 秒后进入持久化，同批后续记录不会重新开始期限。系统与线程调度、"
        + "SQLite I/O 或故障可能使实际完成时间稍晚，这不是严格的五秒保证。进程异常终止时仍可能"
        + "丢失最多 63 条尚未提交记录；队列溢出、超大记录或写入失败也会造成缺口。会话完成记录只"
        + "尝试保存一次且不自动重试；保存失败时会话显示 Error，已经成功提交的记录仍会保留。"
        + "没有完成记录的会话表示本次接入可能异常中断。本程序不上传任何数据。实时历史、只读派生"
        + "关联/删除会话聚合/风险分析，以及独立 live-owned 规范投影，均只在对应页面由用户主动"
        + "查看或执行；它们不会自动写入或伪装成离线“删除事件”“原始证据”、离线身份、序号或"
        + "哈希链。SQLite 和实时连续性哈希均不是防篡改介质，"
        + "本功能仍不是完整或生产级取证系统。";
