...
public sealed class LiveMonitoringViewModel : ViewModelBase, IDisposable
{
    ...
    public const string PersistenceDisclosure =
        "Phase 2B.2.1 实时接入：你手动开始后，本次接收到的受支持事件原始 XML、解析与分类结果"
        ...
        + "没有完成记录的会话表示本次接入可能异常中断。本程序不上传任何数据；尚无实时历史查看界面。"
        + "实时关联、删除会话聚合和风险评估尚未接入，"
        + "新保存的实时明细也尚未投影到“删除事件”或“原始证据”页面。SQLite 不是防篡改介质，"
        + "本功能仍不是完整或生产级取证系统。";
