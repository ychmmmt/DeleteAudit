...
| 2 | 项目级 | 没有 Live History 只读界面 | 产品边界 | **保留** | 属 Phase 2B.2.2 目标，本轮未实现 |
...
```text
本轮关闭：7, 8, 9, 10, 11, 12, 13，以及 14 的应用侧
仍然保留：1, 2, 3, 4, 5, 6, 9b, 14（外部写入者）
```

**项目当前 P2 总数 = 8**，与审计报告的 `P2 = 8` 数值巧合相同，但**构成完全不同**：审计那 8 项
中已有 6 项关闭、1 项部分关闭，其余名额由此前一直存在的产品级债务占据。
