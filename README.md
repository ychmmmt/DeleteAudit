...
- **实时接入（Phase 2B.1）**：在"实时接入预览"页点击开始后，只读订阅本机已有的日志通道。**你手动开始之后，本次收到的受支持事件的原始 XML 与分类结果会写入本机的 SQLite 数据库**；已成功写入的明细在停止或关闭应用后**会保留**。同时保存监控会话摘要。
...
- **保存可能存在缺口。** 队列溢出、超大事件、写入失败或进程异常终止都会造成缺口；低流量时最多有 63 条已解析记录暂存在内存里，直到攒满一批或你停止监控才写盘。没有完成记录的会话表示本次接入可能异常中断。
