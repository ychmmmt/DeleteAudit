...
DeleteAudit 是一款面向 Windows 的开源日志查看与整理工具。它可以导入受支持的本地日志文件，帮助你查看导入结果，并在你明确开启后接入、保存和浏览本机实时日志。项目目前仍处于 Alpha 阶段，适合学习、测试和验证工作流，**不应被视为完整或生产级取证系统**。
...
- **实时接入（Phase 2B）**：在“实时接入预览”页点击开始后，只读订阅本机已有的日志通道。**你手动开始之后，本次收到的受支持事件原始 XML、解析与分类结果及相关实时证据会写入本机的 SQLite 数据库**；已成功写入的明细在停止或关闭应用后**会保留**。同时保存监控会话摘要。
- **实时历史与派生分析**：分页浏览已保存的实时会话、记录、诊断和受限 Raw XML 预览；按需重新解析所选会话，并用与离线导入相同的确定性规则生成只读关联、删除会话和风险视图。
- **独立实时规范投影**：用户显式执行后，把可投影的实时证据写入 `0005` 新增的 live-owned 表，保留 `live_evidence_id`、独立 epoch、密集 live 序号和连续性哈希；不写入、不冒充、也不连接离线事件表、离线身份、离线序号或离线哈希链。
...
- 阶段：**Alpha / experimental**，当前实现阶段为 **Phase 2B**。
- 系统：**Windows 10 / Windows 11**。
- 运行环境：**.NET 8**。
- **不是完整或生产级取证系统**，不达到正式数字取证产品的标准。
- 不能防止误删，也不能防住有意的攻击或证据清除。
- **实时历史、派生关联/会话/风险分析和规范投影现已提供真实查看页面。** 派生分析只读且不写回；规范投影只写 `0005` 的 live-owned 表。它们都不会把实时数据伪装成离线导入，也不会自动出现在离线“删除事件”或“原始证据”页。
- **`0005` migration 必须由开发者或操作员显式应用。** 运行时不会创建或迁移 schema；缺少 `0005` 时只有“实时规范投影”显示 unavailable，既有离线、实时预览、历史和派生分析页面仍可使用。
